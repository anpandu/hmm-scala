...
class HMMTest extends FunSpec with ScalaFutures with TimeLimitedTests {
  ...
  describe("HMM") {
    ...
    it("Basic") {
      ...
      val hmm: HMM = HMM.create(ex_sentences)
      assert(hmm.tags == ex_tags)
      assert(hmm.dict == ex_dict)
      assert(hmm.sentences == ex_new_sentences)
      assert(hmm.threshold == 5)
    }

    it("transformWord") {
      assert("_twoDigitNum_" == HMM.transformWord("69"))
      assert("_fourDigitNum_" == HMM.transformWord("1999"))
      assert("_digitPeriod_" == HMM.transformWord("1.000"))
      assert("_digitPeriod_" == HMM.transformWord("1.000.000"))
      assert("_otherNum_" == HMM.transformWord("123123"))
      assert("_allCaps_" == HMM.transformWord("BBC"))
      assert("_capPeriod_" == HMM.transformWord("M."))
      assert("_initCap_" == HMM.transformWord("Sally"))
      assert("_lowerCase_" == HMM.transformWord("can"))
      assert("_firstWord_" == HMM.transformWord("Pertamax", 0))
      assert("_other_" == HMM.transformWord(",", 1))
    }
    ...
    it("createFromCorpus") {
      ...
      val hmm: HMM = HMM.createFromCorpus(path)
      assert(hmm.tags == ex_tags)
      assert(hmm.dict == ex_dict)
      assert(hmm.sentences == ex_new_sentences)
      assert(hmm.threshold == 5)
    ...
    it("export + import") {
      ...
      val hmm2: HMM = HMM.createFromModel(path2)
    ...
    it("countTag, countWordTag") {
      ...
      val hmm: HMM = HMM.createFromModel(path)
    ...
    it("pi, getTagSequence") {
      ...
      assert(hmm.pi(-1, prep_words, "", "") == new TagSequence(1.0, List()))
      assert(hmm.pi(0, prep_words, "_START_", "PRP") == new TagSequence(0.08104718359052518, List("PRP")))
      assert(hmm.pi(1, prep_words, "PRP", "MD") == new TagSequence(0.07688195553769328, List("PRP", "MD")))
      assert(hmm.pi(2, prep_words, "MD", "VBI") == new TagSequence(0.04042320130020522, List("PRP", "MD", "VBI")))
      assert(hmm.pi(3, prep_words, "VBI", ".") == new TagSequence(0.021723771408808535, List("PRP", "MD", "VBI", ".")))
